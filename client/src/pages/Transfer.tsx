...
export default function Transfer() {
  ...
  const handleTransfer = async () => {
    ...
    setLoading(true);
    try {
      await makeTransfer(playerId!, selectedPlayer, parseInt(amount), description || 'Transferência');
  ...
  const formatMoney = (value: number) => `M$ ${(value / 100).toLocaleString('pt-BR', { minimumFractionDigits: 0 })}`;
